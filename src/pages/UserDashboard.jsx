...
export default function UserDashboard() {
  ...
  const loadSkills = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.message || "Your skills could not load.");
      setSkills(data.skills || []);
    } catch (err) {
      setSkillsError(err.message || "Server connection failed.");
    } finally {
  ...
  const stats = useMemo(
    () => ({
      total: skills.length,
      approved: skills.filter((item) => item.status === "approved").length,
      pending: skills.filter((item) => item.status === "pending").length,
    }),
    [skills]
  );

  // Handle File Selection (Gallery / Drag & Drop)
    ...
    <main className="min-h-screen bg-[#faf9ff] px-4 py-10 sm:px-7">
      <section className="mx-auto max-w-7xl">
        ...
        <div className="mt-7 grid gap-6 lg:grid-cols-[240px_1fr]">
          {/* Sidebar Navigation */}
          <aside className="h-fit rounded-3xl border border-violet-100 bg-white p-3.5 shadow-sm space-y-1">
            <button
              type="button"
            ...
            <div className="pt-2 mt-2 border-t border-gray-100 space-y-1">
              <Link
                to="/add-skills"
                className="flex items-center gap-3 rounded-2xl px-4 py-2.5 text-xs font-medium text-gray-600 hover:bg-violet-50 hover:text-violet-800"
              >
          ...
          <div className="space-y-6">
                ...
                <div className="rounded-3xl border border-violet-100 bg-white p-6 shadow-sm sm:p-8">
                  <div className="flex flex-wrap items-start justify-between gap-3 mb-6">
                    ...
                    <button
                      type="button"
                      onClick={loadSkills}
                      className="rounded-xl border border-violet-200 p-2.5 text-violet-700 hover:bg-violet-50 transition"
                      title="Refresh"
                    >
                  ...
                  {skillsError && (
                    <p className="mb-5 rounded-2xl bg-red-50 p-4 text-sm text-red-700 border border-red-100">
                      {skillsError}
                    </p>
                  )}
                  ...
                  {loadingSkills ? (
                    ...
                    <div className="py-16 text-center rounded-2xl border border-dashed border-violet-200 bg-violet-50/40">
                      <Sparkles className="mx-auto text-violet-500 mb-3" size={32} />
                      <h3 className="font-bold text-gray-900 text-lg">No skills yet</h3>
                    ...
                    <div className="space-y-4">
                      {skills.map((item) => (
                        <article
                          key={item._id}
                          className="rounded-2xl border border-gray-100 bg-white p-5 transition hover:border-violet-200 hover:shadow-sm"
                        >
                          <div className="flex flex-wrap items-start justify-between gap-3">
                            <div>
                              <p className="text-lg font-bold text-gray-900">{item.title}</p>
                              <span className="mt-1 inline-block rounded-full bg-violet-50 px-2.5 py-0.5 text-xs font-semibold text-violet-700">
                                {item.category}
                              </span>
                          ...
                          {item.learnSkill && (
                            <div className="mt-3 rounded-xl bg-slate-50 p-2.5 text-xs text-slate-700 flex items-center gap-1.5">
                              <ArrowLeftRight size={13} className="text-violet-600 shrink-0" />
                              <span>Looking to learn: <strong>{item.learnSkill}</strong></span>
                            </div>
                          )}

                          <div className="mt-3 rounded-xl bg-violet-50/70 p-3 text-xs text-violet-950">
            ...
            {activeTab === "profile" && (
              <div className="rounded-3xl border border-violet-100 bg-white p-6 shadow-sm sm:p-8">
                ...
                {profileSuccess && (
                  <div className="mt-5 rounded-2xl bg-emerald-50 p-4 text-sm font-semibold text-emerald-800 border border-emerald-200 flex items-center gap-2">
                ...
                <form onSubmit={handleSaveProfile} className="mt-6 space-y-6">
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-5">
                      ...
                      <div className="relative shrink-0">
                        {avatarPreview ? (
                          ...
                          <div className="w-24 h-24 rounded-full bg-violet-100 flex items-center justify-center text-violet-700 font-bold text-3xl shadow-xs">
                      ...
                      >
                        <input
                          ...
                          className="hidden"
                        />
                        <Upload size={22} className="text-violet-600 mb-1.5" />
                        <p className="text-xs font-semibold text-gray-800">
                          Drag & drop your picture here, or <span className="text-violet-600 underline">choose from gallery</span>
                        </p>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full rounded-xl border border-gray-300 px-3.5 py-2.5 text-sm text-gray-900 outline-none focus:border-violet-600 focus:ring-1 focus:ring-violet-600"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="w-full rounded-xl border border-gray-200 bg-gray-100 px-3.5 py-2.5 text-sm text-gray-500 cursor-not-allowed"
                      />
                    </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full rounded-xl border border-gray-300 p-3 text-sm text-gray-900 outline-none focus:border-violet-600 focus:ring-1 focus:ring-violet-600 leading-relaxed"
                  ...
                  <div className="pt-3">
                    <button
                      ...
                      className="inline-flex items-center justify-center gap-2 rounded-xl bg-violet-600 px-6 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-violet-700 disabled:opacity-50"
                    >
                      {savingProfile ? (
                        <>
              ...
              <div className="rounded-3xl border border-violet-100 bg-white p-6 shadow-sm sm:p-8">
                ...
                {loadingMessages ? (
                  ...
                  <div className="py-16 text-center rounded-2xl border border-dashed border-violet-200 bg-violet-50/40">
                    ...
                    <h3 className="font-bold text-gray-900 text-lg">No active messages</h3>
                    <p className="mt-1 text-sm text-gray-500 max-w-sm mx-auto">
                  ...
                  <div className="divide-y divide-gray-100">
                    {conversations.map((conv) => (
                      <div
                        key={conv.room}
                        className="py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-violet-50/40 rounded-xl px-3 transition"
                      >
                        <div className="flex items-center gap-3.5 min-w-0">
                          {conv.avatar ? (
                            <img
                              ...
                              className="w-11 h-11 rounded-full object-cover shrink-0 border border-violet-100"
                            />
                          ) : (
                            <div className="w-11 h-11 rounded-full bg-violet-100 text-violet-700 font-bold flex items-center justify-center text-base shrink-0">
                              {conv.recipientName?.charAt(0) || "U"}
                            </div>
                          )}
                          <div className="min-w-0">
                            <h4 className="font-bold text-gray-900 text-sm">{conv.recipientName}</h4>
                            <p className="text-xs text-gray-500 truncate max-w-md">{conv.lastMessage}</p>
                        ...
                        <Link
                          ...
                          className="inline-flex items-center justify-center gap-1.5 rounded-xl bg-violet-50 px-4 py-2 text-xs font-semibold text-violet-700 hover:bg-violet-100 transition shrink-0"
                        >
...
function StatCard({ label, value, icon }) {
  return (
    <div className="rounded-2xl border border-violet-100 bg-white p-5 shadow-sm flex items-center justify-between">
      <div>
        <p className="text-3xl font-bold text-gray-900">{value}</p>
        <p className="mt-1 text-xs font-semibold uppercase tracking-wider text-gray-500">{label}</p>
      </div>
      <div className="rounded-2xl bg-violet-50 p-3">{icon}</div>
    </div>
  );
}
